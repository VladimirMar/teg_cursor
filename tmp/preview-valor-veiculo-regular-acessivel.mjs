...
WITH parametro_viagem AS (
  ...
  SELECT
    v.codigo,
    ROUND(((fixo.valor_fixo * 12) + (12 * parametro_viagem.viagem * parametro_capacidade.capacidade * percap.valor_percap)), 2) AS valor_calculado
  FROM veiculo v
...
SELECT
  (SELECT viagem::text FROM parametro_viagem) AS viagem,
...
SELECT
  ...
  ROUND(((fixo.valor_fixo * 12) + (12 * parametro_viagem.viagem * parametro_capacidade.capacidade * percap.valor_percap)), 2)::text AS valor_calculado
FROM veiculo v
CROSS JOIN parametro_viagem
CROSS JOIN parametro_capacidade
CROSS JOIN fixo
