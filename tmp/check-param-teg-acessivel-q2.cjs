...
(async () => {
  ...
  const result = await client.query(`
    ...
    WHERE translate(upper(trim(m.descricao)),'ÁÀÃÂÉÊÍÓÔÕÚÇ','AAAAEEIOOOUC') = translate(upper('TEG ACESSIVEL'),'ÁÀÃÂÉÊÍÓÔÕÚÇ','AAAAEEIOOOUC')
  ...
  console.log('PARAM_TEG_ACESSIVEL_Q2:', JSON.stringify(result.rows, null, 2));
  await client.end();
})().catch((error) => {
