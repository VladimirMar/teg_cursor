...
WITH veiculos AS (
  ...
  WHERE ${normalizeSqlText.replace('%s', 'os_especial')} = 'NAO'
    AND ${normalizeSqlText.replace('%s', 'tipo_de_bancada')} = 'CRECHE'
  ...
  WHERE ${normalizeSqlText.replace('%s', 'm.descricao')} = 'TEG CRECHE'
    AND ${normalizeSqlText.replace('%s', 'tb.descricao')} = 'CRECHE'
    AND pv.data = (
      SELECT MAX(pv2.data)
  ...
  SELECT
    ...
    p1.condicao_key AS parametro_multiplicador,
    p2.condicao_key AS parametro_subtracao_a,
    p3.condicao_key AS parametro_subtracao_b,
    COUNT(*) FILTER (
      WHERE ROUND(((pares_valores.valor_fixo * 12) + ((((veiculos.cap_teg * p1.valor) - (p2.valor * p3.valor)) * pares_valores.valor_percap) * 12)), 2) = veiculos.valor_veiculo
    )::int AS total_iguais,
    COUNT(*)::int AS total_veiculos,
    MIN(ROUND(((pares_valores.valor_fixo * 12) + ((((veiculos.cap_teg * p1.valor) - (p2.valor * p3.valor)) * pares_valores.valor_percap) * 12)), 2))::text AS menor_calculado,
    MAX(ROUND(((pares_valores.valor_fixo * 12) + ((((veiculos.cap_teg * p1.valor) - (p2.valor * p3.valor)) * pares_valores.valor_percap) * 12)), 2))::text AS maior_calculado
  FROM veiculos
  CROSS JOIN pares_valores
  CROSS JOIN parametros p1
  CROSS JOIN parametros p2
  CROSS JOIN parametros p3
  GROUP BY pares_valores.condicao_key, p1.condicao_key, p2.condicao_key, p3.condicao_key
)
SELECT *
FROM candidatos
ORDER BY total_iguais DESC, condicao_valor ASC, parametro_multiplicador ASC, parametro_subtracao_a ASC, parametro_subtracao_b ASC;
`
