...
(async () => {
  ...
  const result = await client.query(`
    WITH base AS (
      ...
      GROUP BY aps.data_referencia, aps.mes_ano, aps.dre_codigo, aps.ordem_servico_codigo, aps.revisao, coalesce(trim(aps.tipo_pessoa),'')
    )
    SELECT *
    FROM base
    WHERE qtd_acessivel_acm = 2
    ORDER BY data_referencia, ordem_servico_codigo
    LIMIT 30
  `);

  console.log('BUTANTA_QTD_ACESSIVEL_ACM_2:', JSON.stringify(result.rows, null, 2));
  await client.end();
})().catch((error) => {
