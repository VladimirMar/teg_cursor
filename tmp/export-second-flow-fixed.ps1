...
$cmdT = $conn.CreateCommand(); $cmdT.CommandText = 'SELECT * FROM [Credenciados] ORDER BY 1'; $rt = $cmdT.ExecuteReader()
$rows = New-Object System.Collections.Generic.List[object]
while($rt.Read()){
  $rows.Add([PSCustomObject]@{ Codigo=(ToVal $rt.GetValue(0)); Titular=(ToVal $rt.GetValue(1)); CnpjCpf=(ToVal $rt.GetValue(2)) })
}
$rt.Close()
