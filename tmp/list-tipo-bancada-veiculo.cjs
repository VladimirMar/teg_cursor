...
(async () => {
  const res = await pool.query(`
    SELECT COALESCE(NULLIF(BTRIM(tipo_bancada), ''), '<vazio>') AS tipo_bancada, COUNT(*)::int AS total
    FROM veiculo
    GROUP BY 1
    ORDER BY CASE WHEN COALESCE(NULLIF(BTRIM(tipo_bancada), ''), '<vazio>') = '<vazio>' THEN 1 ELSE 0 END, 1
  `);
