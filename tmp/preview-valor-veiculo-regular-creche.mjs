...
WITH parametro_multiplicador AS (
  ...
  WHERE ${normalizeSqlText.replace('%s', 'm.descricao')} = 'TEG CRECHE'
    ...
    AND ${normalizeSqlText.replace('%s', 'c.descricao')} = '11 EM DIANTE'
  ...
  SELECT
    v.codigo,
    ROUND(((fixo.valor_fixo * 12) + ((((v.cap_teg::numeric(14,2) * parametro_multiplicador.multiplicador) - (parametro_subtracao_a.subtracao_a * parametro_subtracao_b.subtracao_b)) * percap.valor_percap) * 12)), 2) AS valor_calculado
  FROM veiculo v
  ...
  WHERE ${normalizeSqlText.replace('%s', 'v.os_especial')} = 'NAO'
    AND ${normalizeSqlText.replace('%s', 'v.tipo_de_bancada')} = 'CRECHE'
    AND v.cap_teg IS NOT NULL
)
SELECT
  (SELECT multiplicador::text FROM parametro_multiplicador) AS multiplicador,
  (SELECT subtracao_a::text FROM parametro_subtracao_a) AS subtracao_a,
  (SELECT subtracao_b::text FROM parametro_subtracao_b) AS subtracao_b,
...
SELECT
  ...
  ROUND(((fixo.valor_fixo * 12) + ((((v.cap_teg::numeric(14,2) * parametro_multiplicador.multiplicador) - (parametro_subtracao_a.subtracao_a * parametro_subtracao_b.subtracao_b)) * percap.valor_percap) * 12)), 2)::text AS valor_calculado
FROM veiculo v
CROSS JOIN parametro_multiplicador
CROSS JOIN parametro_subtracao_a
CROSS JOIN parametro_subtracao_b
